/*
 * File:   SensorExactoPosNeg.c
 * Author: Luis A-Nuñez
 *
 * Created on 12 de abril de 2021, 04:32 PM
 */
//SENSOR DE TEMPERATURA POSITIVA Y NEGATIVA: VALORES EXACTOS.
...
//Para lectura de temperatura negativa.
int ADC1 = 0;
float VoltajeA = 0;
float VoltajeB = 0;
float temperatura = 0;
...
{
    //Voltage references.
    ADCON1bits.PVCFG = 0b00;    //Positive voltage to A/D is reserved.
    ADCON1bits.NVCFG = 0b00;    //Negative voltage to A/D is reserved. 
...
//Toma los datos analogicos, que vienen del pin an1
short Read_ADC1(char canal) 
{
	...
	{
        case 0: 
            ADCON0 = 0b00001010;    //Inicia la conversion en el canal AN-2.
            //Bit 7:    siempre se lee como 0.
            //Bit 6-2:  Se selecciona el canal analogico.
            //Bit 1:    Se inicia con la conversion.
            //Bit 0:    el ADC esta deshabilitado.
...
void SensorLM35(void)
{
    ...
    //Se leen los canles analogicos AN0 Y AN1.
    ADC = Read_ADC(0)/64;           //Despliega los 8 bits menos significativos.
    ADC1 = Read_ADC1(2)/64;
    //Se toman los datos del LM35.
    R_LM35 = 500.01 / 1024;       //Formula suministraada en el datasheet del LM35.
        //499.9 es en referencia a los 500mV que se suministran al pic para operar.
    ...
    if(temperatura >= 0)
    {
        ...
        {
            putcm(0x80);
            sprintf(Valores, "Temp. =%3.2f %cC",ceil(temperatura), 0xDF);
            __delay_ms(10);    //Para mostrar una vez inicializada la LCD.
            printf(Valores);
            __delay_ms(10);    //Se actualizan los valores cada X mS.
        }
        else
        {
            putcm(0x80);
            sprintf(Valores, "Temp. =%3.2f %cC",floor(temperatura), 0xDF);
            __delay_ms(10);    //Para mostrar una vez inicializada la LCD.
            printf(Valores);
            __delay_ms(10);    //Se actualizan los valores cada X mS.
        }
    ...
    if(temperatura >= 150)
    {
        ...
        __delay_ms(1000);    //Se actualizan los valores cada X mS.
    }
    if(temperatura <= -54)
    {
        putcm(0x80);
...
void Mostrar_Reloj()
{
	...
	//putcm(0xC2);
	//printf("%d%d:%d%d:%d%d",Decenas_Horas, Unidades_Horas, Decenas_Minutos, Unidades_Minutos, Decenas_Segundos, Unidades_Segundos);
...
void Cronometro()
{
    ...
    TMR0L = 0xFF;       //Evita que al dejar de presionar el boton de ajuste el contador se reinicie a su estado de inicializacion.
    __delay_ms(1000);
}
...
void Alarma()
{
    ...
    else
    {
        ...
        printf(" Modo Alarma ");
        __delay_ms(1000);
        //Deshabilitamos interupciones globales.
        INTCONbits.GIE = 1;         //Se habilitan las interrupciones globales.
        INTCONbits.PEIE = 1;        //Se habilitan las interrupciones por perifericos.
        ...
        INTCONbits.PEIE = 1;    //Habilita las interrupciones meiante bbotones.
    }
}
